...
constexpr char c_min_delay = 60;
constexpr char c_max_timeouts = 16;
...
static void set_timeout(void *callback, uint32_t timeout, uint8_t cls) {
	...
	for (; timeouts[k].callback; k = (k + 1) & (c_max_timeouts - 1));
...
static void update_timeouts(void) {
    ...
    if (UNLIKELY(diff || UNLIKELY(ocra < c_min_delay || ocra - c_min_delay < tcnt))) {
        ocra = tcnt + c_min_delay;
    }
