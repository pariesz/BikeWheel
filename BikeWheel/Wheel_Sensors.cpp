...
namespace Wheel_Sensors {
    ...
    inline uint16_t get_gyro_angle(uint32_t time_diff) {
        ...
        int16_t gyro_dist = (rotation_rate * (int32_t)time_diff) / 500000;
    ...
    void update() {
        ...
        static uint8_t combine_accum = COMBINE_STEP;
        ...
        // for performance only update from sensor every COMBINE_STEP updates
        if (++combine_accum < COMBINE_STEP) {
        ...
        */
        ...
        // gyro is signed int32 at a scale of +-2000deg/s
        ...
        // combination filter
        // 75% gyro + 25% acc
        angle = (gyro_angle >> 1) + (gyro_angle >> 2) + (acc_angle >> 2);
        combine_accum = 0;
